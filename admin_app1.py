...
# 🔄 Refresh Button - must be at the top of the sidebar
...
DB_PATH = "production_app.db"
SALT = "static_salt_change_me"   # same salt everywhere
...
def send_email(to_email, subject, body):
    sender_email   = "hafsatuxy@gmail.com"
    brevo_login    = "96cf99001@smtp-brevo.com"
    brevo_password = "OKnmRy6V7fUY509I"
    smtp_server    = "smtp-relay.brevo.com"
    smtp_port      = 587
    try:
        msg = MIMEMultipart()
...
def create_admin(username, password, email):
    ...
    # Remove any existing username to avoid duplicates
    cur.execute("DELETE FROM users WHERE username = ?", (username,))
    cur.execute("""
        INSERT INTO users (username, password_hash, email,
                           is_admin, is_verified, created_at)
        VALUES (?, ?, ?, 1, 1, ?)
    """, (username, hashed_pw, email, datetime.utcnow().isoformat()))
    conn.commit()
    conn.close()
    ...
    if st.sidebar.button("Login", key="login_btn"):
        ...
        if ok:
            ...
            if user.get("is_admin") == 1 or user.get("is_admin") is True:
                ...
                conn.commit()
                user["is_verified"] = 1  # update the local dict too
...
if auth.get("logged_in"):
    ...
    with st.sidebar.expander("⚙️ Deferment Controls", expanded=False):
        ...
        options = ["None", "Maintenance", "Pipeline Issue", "Reservoir", "Other"]
        for i in range(1, 6):  # reduced to 5 in UI for practicality
            st.markdown(f"**Deferment {i}**")
            reason = st.selectbox(f"Reason {i}", options, key=f"deferment_reason_{i}")
            start_date = st.date_input(f"Start Date {i}", key=f"deferment_start_{i}")
        ...
        st.markdown("### 📋 Active Deferments")
        active = {
            k: v for k, v in st.session_state["deferments"].items()
            if v["reason"] != "None" and v["duration_days"] > 0
        }
        if active:
            for name, d in active.items():
                st.markdown(
                    f"- **{name}** → {d['reason']} starting {d['start_date']} "
                    f"for {d['duration_days']} days"
                )
        else:
            st.info("No active deferments set")
    ...
    with st.sidebar.expander("⚡ Ramp-up & Decline Controls", expanded=False):
        ...
        ramp_rate = st.number_input(
            "New Ramp-up Rate (absolute)", min_value=0.0, value=5000.0, key="ramp_rate"
        )
        if st.button("Apply Ramp-up"):
            st.session_state["rampups"][f"ramp_{len(st.session_state['rampups'])+1}"] = {
                "date": str(ramp_date),
        ...
        if "target_avg" not in st.session_state:
            st.session_state["target_avg"] = {"Oil": None, "Gas": None, "lock": True}
        ta_oil = st.number_input(
            "Target Avg Oil (BOPD) — leave 0 to ignore",
            min_value=0,
            value=int(st.session_state["target_avg"].get("Oil") or 0),
            key="ta_oil"
        )
        ta_gas = st.number_input(
            "Target Avg Gas (MMSCFD) — leave 0 to ignore",
            min_value=0,
            value=int(st.session_state["target_avg"].get("Gas") or 0),
            key="ta_gas"
        )
        lock_sched = st.checkbox(
            "Lock scheduled points (do not rescale them)",
            value=st.session_state["target_avg"].get("lock", True)
        )
        if st.button("Set Target Avg"):
            st.session_state["target_avg"]["Oil"] = int(ta_oil) if ta_oil > 0 else None
            st.session_state["target_avg"]["Gas"] = int(ta_gas) if ta_gas > 0 else None
    ...
    # Not logged in → you can show a message or hide completely
    st.sidebar.info("🔒 Login to access production controls.")
...
with tabs[2]:
    ...
    if user and user.get('is_admin'):
        ...
        # Admin ability to create new admins
        import sqlite3
        from datetime import datetime
        import streamlit as st
        import hashlib
        ...
        DB_PATH = "production_app.db"
        SALT = "static_salt_change_me"   # same salt used everywhere
        ...
        def create_admin(username, password, email):
            ...
            # Remove any existing user with same username to avoid duplicates
            cur.execute("DELETE FROM users WHERE username = ?", (username,))
            ...
            conn.commit()
            conn.close()
            return f"✅ New admin '{username}' created and verified."

        # ----------------- STREAMLIT UI -----------------
        if user and user.get("is_admin"):     # only show to admins
            ...
            with st.form("add_admin_form"):
                ...
                submit_new = st.form_submit_button("Create Admin")
            ...
            if submit_new:
                if not new_user or not new_email or not new_pass:
                    ...
                    msg = create_admin(new_user, new_pass, new_email)
                    st.success(msg)
        else:
            st.info("🔒 Admin privileges required to create new admins.")
